...
static bool chery_tx_hook(const CANPacket_t *msg) {
  ...
  if (msg->addr == CHERY_LANE_KEEP) {
    ...
    const int zone_max_angle_can = (int)(safety_interpolate(CHERY_ZONE_ANGLE_DEG, speed_ms) * (float)CHERY_STEERING_LIMITS.angle_deg_to_can);
    violation |= safety_max_limit_check(desired_angle, zone_max_angle_can, -zone_max_angle_can);
  }
