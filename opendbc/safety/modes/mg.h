...
// MG ZS (both EV and non-EV/ICE 2025 variants) - ported from dragonpilot's
// opendbc (MIT), commit ca9cafcb4 "MG: add ZS (ICE) support". Message names
// in comments below (SCS_HSC2_FrP19, EPS_HSC2_FrP03, etc.) are the vehicle's
// own real manufacturer signal naming (HSC2 = High Speed CAN 2), not
// generic placeholders - opendbc/dbc/mg.dbc is a genuine capture, not a
// template. Steering uses the standard, widely-used driver-torque-limited
// mechanism (steer_torque_cmd_checks / TorqueSteeringLimits, same one Honda
// and many other brands use) - no novel safety-check design needed here,
// unlike Chery's angle-based backstop.
//
// Dropped from the source: the `alka_allowed`/`lkas_on`/`ALT_EXP_ALKA`
// lines (dragonpilot's own "ALKA" - lane keep without engagement - opt-in
// feature). That framework doesn't exist in this fork's safety.h/lateral.h
// at all, so those lines would not compile; omitting them is a real,
// documented feature drop (not a silent behavior change from what this
// fork could otherwise do), not a bug.
