      ...
      for attr in dir(test):
        ...
        if isinstance(tc, type) and issubclass(tc, SafetyTest) and attr != current_test:
          ...
          if tx is not None and not attr.endswith('Base'):
            ...
            # Volkswagen MQB GRA_ACC_01 (cancel/resume) and GWM's steering command overlap at 0x12B
            if attr.startswith('TestVolkswagenMqb') and current_test == 'TestGwmSafety':
              tx = list(filter(lambda m: m[0] not in [0x12B, ], tx))
            if attr == 'TestGwmSafety' and current_test.startswith('TestVolkswagenMqb'):
              tx = list(filter(lambda m: m[0] not in [0x12B, ], tx))
