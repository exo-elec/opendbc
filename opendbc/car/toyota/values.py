...
# Static CAN messages normally sent by the DSU (Driving Support ECU), replayed by openpilot when the DSU
# has been physically disconnected (CarParams.enableDsu) on older TSS-P Toyotas so the rest of the car's
# CAN network (radar, PCS/AEB HUD, etc.) doesn't fault from the DSU's absence. Ported verbatim (addresses,
# bus, send period, and payload bytes) from dev/EDP10's opendbc fork - these are captured real ECU
# payloads, not derived/computed values, so they must not be edited.
STATIC_DSU_MSGS = [
